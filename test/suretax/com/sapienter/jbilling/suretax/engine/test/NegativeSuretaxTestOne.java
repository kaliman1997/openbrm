...
public class NegativeSuretaxTestOne extends DefaultTest {
	...
	public void testWrongTransTypeCode() throws JbillingAPIException,
			IOException {
	...
	/**
	 * We will not be sending zip code, which is a mandatory parameter, when creating a user.
	 * @throws JbillingAPIException
	 * @throws IOException
	 */
	public void testNotSendingRequiredParameter() throws JbillingAPIException,
			IOException {
		long currentTimeMillis = System.currentTimeMillis();
		//
		UserWS user = SureTaxCompositionTaskTest.createUser(currentTimeMillis,
		...
		try {
			...
			if (e instanceof SessionInternalError
					&& e.getMessage().contains("Zip Code is Required")) {
				// do nothing
			} else {
				assertTrue("Wrong exception raised", false);
			}
