...
public abstract class BillingProcessTestCase {
    ...
    public void triggerBilling(Date runDate) {
        ...
        config.setPeriodUnitId(new Integer(Constants.PERIOD_UNIT_DAY));
    ...
    public void assertScenarioChecks(UserWS userC1, int orderIdC1, Date orderNextBillableDate, Date userNextInvoiceDate, 
        ...
        OrderPeriodWS daily = getOrderPeriod(api,allPeriod,Constants.PERIOD_UNIT_DAY,languageId,"Daily");
        OrderPeriodWS weekly = getOrderPeriod(api, allPeriod,Constants.PERIOD_UNIT_WEEK, 1, "Weekly");
        OrderPeriodWS semiMonthly = getOrderPeriod(api,allPeriod,Constants.PERIOD_UNIT_SEMI_MONTHLY,languageId,"Semi-Monthly");
        OrderPeriodWS monthly = getOrderPeriod(api,allPeriod,Constants.PERIOD_UNIT_MONTH,languageId,"Monthly");
        ...
        if ((invoiceIdsWSC1At6Jan.length-previous) != 0){
            ...
            Arrays.sort(invoiceWSC1At6Jan.getInvoiceLines(), new Comparator<com.sapienter.jbilling.server.entity.InvoiceLineDTO>() {
                public int compare(com.sapienter.jbilling.server.entity.InvoiceLineDTO line1, com.sapienter.jbilling.server.entity.InvoiceLineDTO line2) {
