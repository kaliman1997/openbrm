...
target(packageSource: "Packages the source code.") {
    zip(filesonly: false, update: false, destfile: sourcePackageName) {
        zipfileset(dir: basedir, prefix: releaseName) {
            ...
            exclude(name: "**/openbrm.properties")
        ...
        zipfileset(file: "${javaDir}/openbrm.properties.sample", fullpath: "${releaseName}/src/java/openbrm.properties")
    }
}
...
target(updateImage: "Updates the jbilling image with the current release artifacts.") {
    ...
    def jbillingHome = "${imageDir}/openbrm/"
    ...
    copy(file: "${javaDir}/openbrm.properties", tofile: "${jbillingHome}/openbrm.properties", overwrite: true)
