...
public class MediationProcessPartitioner implements InitializingBean, Partitioner {

	private static final FormatLogger logger = new FormatLogger(Logger.getLogger(MediationProcessPartitioner.class));
	...
	public void afterPropertiesSet() throws Exception {
		...
		PluggableTaskManager taskManager = new PluggableTaskManager(entityId,
                SBSConstants.PLUGGABLE_TASK_MEDIATION_PROCESS_FILTER);
		logger.debug("PluggableTaskManager initialized");

        IMediationProcessFilterTask task = (IMediationProcessFilterTask) taskManager.getNextClass();
        ...
        if (task == null) {
        		...
        		//throw exception or default to BasicMediationProcessFilterTask
        }
