...
public class BatchMediationSessionBean implements IMediationSessionBean {
    ...
    public void trigger(Integer entityId) {
        ...
        for (MediationConfiguration cfg : local.getAllConfigurations(entityId)) {
            ...
            LOG.debug("Entering Batch Mediation Process");
            ...
            Job job = (Job) Context.getBean(Context.Name.BATCH_JOB_MEDIATION_PROCESS);
            LOG.debug("Loaded job bean # " + job.toString());
            
            JobParametersBuilder paramBuilder = new JobParametersBuilder();
            paramBuilder.addString(Constants.BATCH_JOB_PARAM_ENTITY_ID, entityId.toString());
            paramBuilder.addString(BatchConstants.BATCH_JOB_PARAM_CONFIG_ID, cfg.getId().toString());
    ...
    public void normalizeRecordGroup(IMediationProcess processTask, Integer executorId,
 	...
 	ArrayList<MediationResult> OthResults = new ArrayList<MediationResult>(0);
	List<Record> othGroup = new ArrayList<Record>(0);
        for (Iterator<Record> it = thisGroup.iterator(); it.hasNext();) {
            Record record = it.next();
           if (hasBeenProcessed(process, record)) {
                MediationResult OthResult = new MediationResult(cfg.getName(),true);
                String error = "ERR_DUPLICATE_RECORD";
                OthResult.setDone(true);
                OthResult.addError(error);
                OthResult.setRecordKey(record.getKey());
                OthResults.add(OthResult);   
		othGroup.add(record);
                it.remove();
            }
	...
	for (MediationResult oResult : OthResults) {
		
		LOG.debug("Record those not passed to Rating");
		handleMediationErrors(findRecordByKey(othGroup, oResult.getRecordKey()), oResult.getErrors(), entityId, cfg);
