...
public class ExclusionDAS extends AbstractDAS<ExclusionDTO>{

private static final Logger LOG = Logger.getLogger(ExclusionDAS.class);

	
  	final String prefixHql =
                "select r " +
                "from international_ex_rate r " +
	...
	public List<ExclusionDTO> findByPrefix(String prefix) {
		Criteria criteria = getSession().createCriteria(ExclusionDTO.class);
	...
	LOG.debug("In RateDAS values are:"+prefix+","+pdate+","+itemId);
	 ...
	 public List<ExclusionDTO> findByEntity(Integer entityId, Integer limit) {
