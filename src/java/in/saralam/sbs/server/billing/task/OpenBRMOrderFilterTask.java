 ...
 * for Date types
 */
public class OpenBRMOrderFilterTask 
        extends PluggableTask implements OrderFilterTask {

    private static final FormatLogger LOG =  new FormatLogger(Logger.getLogger(OpenBRMOrderFilterTask.class));
    protected Date billingUntil = null;
    
    public OpenBRMOrderFilterTask() {
        billingUntil = null;
    }
    ...
    public boolean isApplicable(OrderDTO order, 
        ...
        try {
            ...
            if (order.getBillingTypeId().compareTo(
				...
				/* Filter usage order's first
				* we should only bill usage until start of the billing period. 
				* Start date is exclusive
				* ex:
				* Mar 1 billing will only consider usage order's created until 28th Feb but 1st Mar
				*/
				
				if(order.getNotes() != null && order.getNotes().startsWith("Usage Order")) {
					//it is an usage order
					Date billingFrom = process.getBillingDate();
					LOG.warn("Billing date is " +  billingFrom);
					if(activeSince.equals(billingFrom) || activeSince.after(billingFrom) ) {
						LOG.warn("Excluding Usage order with active since " + activeSince);
						retValue = false;
						excludeUsage = true;
						return retValue;
					}
                ...
                // check if it is too early 
				 if(excludeUsage || activeSince.equals(billingUntil) ||
                        activeSince.after(billingUntil)) {
