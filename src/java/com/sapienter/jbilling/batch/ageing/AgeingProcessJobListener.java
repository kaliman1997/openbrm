...
public class AgeingProcessJobListener implements JobExecutionListener {
	...
	private final FormatLogger logger = new FormatLogger(Logger.getLogger(AgeingProcessJobListener.class));
	...
	public void afterJob(JobExecution jobExec) {
		...
		Integer entityId = Integer.parseInt(jobExec.getJobParameters().getString(Constants.BATCH_JOB_PARAM_ENTITY_ID));
	...
	public void beforeJob(JobExecution jobExec) {
		...
		Date ageingDate = jobParams.getDate(Constants.BATCH_JOB_PARAM_AGEING_DATE);
		...
		jobExec.getExecutionContext().put(Constants.JOBCONTEXT_USERS_LIST_KEY, loadAndSort(entityId, ageingDate));
	}
     ...
     * 
     * @param entityId
     *            : company id for which users will be found
     * @param billingDate
     *            : billing date of users
     */
