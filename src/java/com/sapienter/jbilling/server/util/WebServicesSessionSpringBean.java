...
import in.saralam.sbs.server.crm.TicketBL;
...
import dk.comtalk.billing.server.customer.balance.UserBalanceBL;
...
import in.saralam.sbs.server.RateCard.RateBL;
...
import in.saralam.sbs.server.PriceModel.PriceModelBL;
...
import in.saralam.sbs.server.RumMap.RumMapBL;
...
import in.saralam.sbs.server.PriceMap.PriceMapBL;
...
import in.saralam.sbs.server.ConvergentRateCard.CRateBL;
...
import in.saralam.sbs.server.openRate.destinationMap.DestinationMapBL;
...
import in.saralam.sbs.server.openRate.worldZoneMap.WorldZoneMapBL;
...
import in.saralam.sbs.server.pricing.PricePackageBL;
...
import in.saralam.sbs.server.notification.OpenBRMNotificationTask;
...
public class WebServicesSessionSpringBean implements IWebServicesSessionBean {
	...
	public Integer createUserWithCompanyId(UserWS newUser, Integer companyId)
		...
		if (0 == newUser.getMainRoleId().compareTo(Constants.TYPE_CUSTOMER)) {
			if (null == newUser.getAccountTypeId()) {
				throw new SessionInternalError(
						"Customer users must have account type id defined.",
	...
	public Integer[] getCustomersLinkedToUser(Integer userId)
			throws SessionInternalError {
		return getAssociatedObjectsByUserAndType(userId,
				UserCodeObjectType.CUSTOMER);
	}
	...
	private void validateItem(ItemDTOEx item) {
		...
		for (InternationalDescriptionWS description : descriptions) {
			if (!description.isDeleted()
					&& StringUtils.isNotEmpty(description.getContent().trim())) {
				noDescriptions = false;
				break;
			}
		}
	...
	public String deleteOrder(Integer id) throws SessionInternalError {
		...
		if (getOrder(id).getGeneratedInvoices().length > 0) {
			throw new SessionInternalError(
					"Error on delete Order ",
					new String[] { "order.have.invoice.cannot.be.delete," + id });
		}
	...
	public boolean updateAccountType(AccountTypeWS accountType)
		...
		AccountTypeWS existing = AccountTypeBL.getWS(new AccountTypeDAS()
				.find(accountType.getId()));
		...
		AccountTypeDTO accountTypeDTO = AccountTypeBL.getDTO(accountType,
				entityId);
	...
	public AccountInformationTypeWS[] getInformationTypesForAccountType(
		...
		if (accountInformationTypes.size() > 0) {
			for (AccountInformationTypeDTO ait : accountInformationTypes) {
				AccountInformationTypeWS accountInformationTypeWS = AccountInformationTypeBL
						.getWS(ait);
	...
	public Integer createAccountInformationType(
		...
		AccountInformationTypeDTO dto = AccountInformationTypeBL.getDTO(
				accountInformationType, getCallerCompanyId());
	...
	private List<InternationalDescriptionWS> getAllDiscountDescriptions(
		...
		for (InternationalDescriptionDTO descriptionDTO : descriptionsDTO) {
			descriptionsWS.add(DescriptionBL
					.getInternationalDescriptionWS(descriptionDTO));
		}
	...
	public void validateDiscountLines(OrderDTO order,
			List<OrderChangeDTO> orderChanges) throws SessionInternalError {
		...
		for (DiscountLineDTO discountLine : order.getDiscountLines()) {
			...
			if (discountLine.isOrderLevelDiscount()) {
				...
				// discount should not be active compared to order
					...
					throw new SessionInternalError(
							"Discount Start Date is in future w.r.t. order's active since date.",
							new String[] { "DiscountWS,startDate,discount.startDate.in.future,"
									+ discount.getCode()
									+ ","
									+ com.sapienter.jbilling.server.util.Util
											.formatDate(
				...
				// Discount should be active till order active since date
					...
					throw new SessionInternalError(
							"Discount End Date is in before the order active since date.",
				...
				if ((discount.getStartDate() != null && orderChange != null)
						&& discount.getStartDate().after(
								orderChange.getStartDate())) {
					throw new SessionInternalError(
							"Discount Start Date is in future w.r.t. order change's start-date of item.",
				...
				if ((discount.getEndDate() != null && orderChange != null)
						&& discount.getEndDate().before(
								orderChange.getStartDate())) {
					throw new SessionInternalError(
							"Discount End Date is in before the order-change start date of item.",
	...
	public Integer createUpdateCommissionProcessConfiguration(
		...
		CommissionProcessConfigurationDTO dto = CommissionProcessConfigurationBL
				.getDTO(ws);
	...
	public CommissionProcessRunWS[] getAllCommissionRuns() {
		...
		if (commissionProcessRuns != null && commissionProcessRuns.size() > 0) {
			...
			for (CommissionProcessRunDTO commissionProcessRun : commissionProcessRuns) {
				commissionProcessRunWSes[index] = CommissionProcessConfigurationBL
						.getCommissionProcessRunWS(commissionProcessRun);
				index++;
			}
	...
	public CommissionWS[] getCommissionsByProcessRunId(Integer processRunId) {
		...
		if (commissions != null && commissions.size() > 0) {
			...
			for (CommissionDTO commission : commissions) {
				commissionWSes[index] = CommissionProcessConfigurationBL
	...
	public void updatePreference(PreferenceWS preference) {
		...
		if (preference.getPreferenceType().getId() == Constants.PREFERENCE_PARTNER_DEFAULT_COMMISSION_TYPE
				&& StringUtils.isNotBlank(preference.getValue().trim())) {
			preference.setValue(StringUtils.trim(preference.getValue()
					.toUpperCase()));
			if (!(PartnerCommissionType.INVOICE.name().equals(
					preference.getValue()) ? true
					: (PartnerCommissionType.PAYMENT.name().equals(
							preference.getValue()) ? true : false))) {
				String errorMessages[] = new String[1];
	...
	public CurrencyWS[] getCurrencies() {
		...
		for (CurrencyDTO currency : currencies) {
			ws.add(CurrencyBL.getCurrencyWS(currency,
					(currency.getId() == entityDefault)));
		}
	...
	// MediationErrorWS
	public List<MediationErrorWS> getUsageErrors(String fromDate,
			String toDate, int limit) throws SQLException, NamingException {
		MediationUtil mediationutil = new MediationUtil();
		return mediationutil.getMediationErrorsByDate(fromDate, toDate, limit);
	}
	...
	public void triggerMediationAsync() {
		final Integer companyId = getCallerCompanyId();
		Thread t = new Thread(new Runnable() {
			IMediationSessionBean processBean = Context
					.getBean(Context.Name.MEDIATION_SESSION);

			public void run() {
				processBean.trigger(companyId);
			}
		});
	...
	public List<MediationProcessWS> getAllMediationProcesses() {
		IMediationSessionBean mediationBean = Context
				.getBean(Context.Name.MEDIATION_SESSION);
		List<MediationProcess> processes = mediationBean
				.getAll(getCallerCompanyId());

		// convert to web-service mediation process
		List<MediationProcessWS> ws = new ArrayList<MediationProcessWS>(
				processes.size());
	...
	public List<MediationRecordLineWS> getMediationEventsForOrder(
		...
		IMediationSessionBean mediationBean = Context
				.getBean(Context.Name.MEDIATION_SESSION);
		List<MediationRecordLineDTO> events = mediationBean
				.getMediationRecordLinesForOrder(orderId);

		return MediationRecordBL.getWS(events);
	}

	public List<MediationRecordLineWS> getMediationEventsForInvoice(
			Integer invoiceId) {
		List<MediationRecordLineDTO> events = new MediationRecordLineDAS()
	...
	public List<MediationRecordWS> getMediationRecordsByMediationProcess(
		...
		IMediationSessionBean mediationBean = Context
				.getBean(Context.Name.MEDIATION_SESSION);
		List<MediationRecordDTO> records = mediationBean
	...
	public List<RecordCountWS> getNumberOfMediationRecordsByStatuses() {
		IMediationSessionBean mediationBean = Context
				.getBean(Context.Name.MEDIATION_SESSION);
		Map<MediationRecordStatusDTO, Long> records = mediationBean
				.getNumberOfRecordsByStatuses(getCallerCompanyId());

		// convert to a simple object for web-services
		List<RecordCountWS> counts = new ArrayList<RecordCountWS>(
				records.size());
		for (Map.Entry<MediationRecordStatusDTO, Long> record : records
				.entrySet())
			counts.add(new RecordCountWS(record.getKey().getId(), record
					.getValue()));
		return counts;
	}
	...
	public List<MediationConfigurationWS> getAllMediationConfigurations() {
		...
		List<MediationConfiguration> configurations = mediationBean
				.getAllConfigurations(getCallerCompanyId());
		...
		for (OrderChangeStatusDTO status : statusDTOs) {
			...
			for (LanguageDTO lang : languages) {
				...
				if (descriptionDTO != null
					...
					ws.addDescription(DescriptionBL
							.getInternationalDescriptionWS(descriptionDTO));
				}
			}
			results.add(ws);
	...
	public Integer createOrderChangeStatus(
			...
			throws SessionInternalError {
		OrderChangeStatusDTO orderChangeStatusDTO = OrderChangeStatusBL
				.getDTO(orderChangeStatusWS);
	...
	public boolean deleteEnumeration(Integer enumerationId)
		...
		if (new MetaFieldDAS().getFieldCountByDataTypeAndName(
				DataType.ENUMERATION, enumerationBL.getEntity().getName(),
				getCallerCompanyId()) > 0
				|| new MetaFieldDAS().getFieldCountByDataTypeAndName(
						DataType.LIST, enumerationBL.getEntity().getName(),
						getCallerCompanyId()) > 0) {
			String[] errors = new String[] { "EnumerationWS,id,enumeration.delete.failed,"
					+ enumerationId };
	...
	// Openrate Module
	// RateCard
	public Integer createRate(RateWS rate) throws SessionInternalError {
		Integer rateId = 0;
		try {
			LOG.debug("in create rate method in wsssb...");
			RateBL bl = new RateBL();
			RateDTO dto = new RateDTO();
			LOG.debug("ratebl.create method is called...");
			rateId = bl.create(rate.getPrefix(), rate.getDestination(),
					rate.getFlatRate(), rate.getconncharge(),
					rate.getscaledRate(), rate.getratePlan(),
					rate.getCreatedDate(), rate.getValidFrom(),
					rate.getValidTo(), rate.getLastUpdatedDate(),
					rate.getRateType(), rate.getEntity());
	...
	public void deleteRate(Integer id) throws SessionInternalError {
	...
	public RateWS getRateWS(Integer rateId) throws SessionInternalError {
		...
		RateDAS das = new RateDAS();
		RateDTO rate = das.find(rateId);
		if (rate == null) {
			return null;
		}
		RateBL bl = new RateBL(rate);
		return bl.getRateWS(languageId);
	}

	public RateDTO[] getRateList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<RateDTO> rates = new RateDAS().findByEntity(entity_id, limit);
		return rates == null ? null : rates.toArray(new RateDTO[rates.size()]);
	}

	// PriceModelRateCard
	public Integer createPrice(PriceModelWS price) throws SessionInternalError {
		Integer priceId = 0;
		try {
			LOG.debug("in create price method in wsssb...");
			PriceModelBL bl = new PriceModelBL();
			PriceModelDTO dto = new PriceModelDTO();
			LOG.debug("pricemodelbl.create method is called...");
			LOG.debug("price.getPriceModel() : " + price.getPriceModel());
			LOG.debug("price.getQtyStep() : " + price.getQtyStep());
			priceId = bl.create(price.getPriceModel(), price.getQtyStep(),
					price.getTierFrom(), price.getTierTo(), price.getBeat(),
					price.getFactor(), price.getChargeBase(),
					price.getPriceModelPlan(), price.getCreatedDate(),
					price.getLastUpdatedDate(), price.getEntity());
	...
	public void deletePriceModel(Integer id) throws SessionInternalError {
	...
	public PriceModelWS getPriceModelWS(Integer priceId)
			throws SessionInternalError {
		...
		PriceModelDAS das = new PriceModelDAS();
		PriceModelDTO price = das.find(priceId);
		if (price == null) {
			return null;
		}
		PriceModelBL bl = new PriceModelBL(price);
		return bl.getPriceModelWS(languageId);
	}

	public PriceModelDTO[] getPriceList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<PriceModelDTO> prices = new PriceModelDAS().findByEntity(
				entity_id, limit);
		return prices == null ? null : prices.toArray(new PriceModelDTO[prices
				.size()]);
	}

	// Price map Ratecard

	public Integer createPriceMap(PriceMapWS price) throws SessionInternalError {
		LOG.debug("create");
		Integer priceMapId = 0;
		try {
			LOG.debug("in create price method in wsssb...");
			PriceMapBL bl = new PriceMapBL();
			PriceMapDTO dto = new PriceMapDTO();
			LOG.debug("pricemapbl.create method is called...");
			LOG.debug("price.getMapGroup() : " + price.getMapGroup());
			LOG.debug("price.getOriginZone() : " + price.getOriginZone());
			priceMapId = bl.create(price.getMapGroup(), price.getOriginZone(),
					price.getDestZone(), price.getZoneResult(),
					price.getTimeResult(), price.getPriceGroup(),
					price.getDescription(), price.getRatePrice(),
					price.getSetUpPrice(), price.getRatingType(),
					price.getPriceMapPlan(), price.getCreatedDate(),
	...
	public void deletePriceMap(Integer id) throws SessionInternalError {
	...
	public PriceMapWS getPriceMapWS(Integer priceMapId)
			throws SessionInternalError {

		if (priceMapId == null || priceMapId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		PriceMapDAS das = new PriceMapDAS();
		PriceMapDTO priceMap = das.find(priceMapId);
		if (priceMap == null) {
			return null;
		}
		PriceMapBL bl = new PriceMapBL(priceMap);
		return bl.getPriceMapWS(languageId);
	}

	public PriceMapDTO[] getPriceMapList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<PriceMapDTO> priceMap = new PriceMapDAS().findByEntity(entity_id,
				limit);
		return priceMap == null ? null : priceMap
				.toArray(new PriceMapDTO[priceMap.size()]);
	}

	// rum map Ratecard

	public Integer createRumMap(RumMapWS rum) throws SessionInternalError {
		Integer rumMapId = 0;
		try {
			LOG.debug("in create rum method in wsssb...");
			RumMapBL bl = new RumMapBL();
			RumMapDTO dto = new RumMapDTO();
			LOG.debug("rummapbl.create method is called...");
			LOG.debug("rum.getPriceGroupp() : " + rum.getPriceGroup());

			rumMapId = bl.create(rum.getPriceGroup(), rum.getStep(),
					rum.getPriceModel(), rum.getRum(), rum.getResource(),
					rum.getResourceId(), rum.getRumType(),
					rum.getConsumeFlag(), rum.getRumMapPlan(),
					rum.getCreatedDate(), rum.getLastUpdatedDate(),
					rum.getEntity());
	...
	public void deleteRumMap(Integer id) throws SessionInternalError {
	...
	public RumMapWS getRumMapWS(Integer rumMapId) throws SessionInternalError {

		if (rumMapId == null || rumMapId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		RumMapDAS das = new RumMapDAS();
		RumMapDTO rumMap = das.find(rumMapId);
		if (rumMap == null) {
			return null;
		}
		RumMapBL bl = new RumMapBL(rumMap);
		return bl.getRumMapWS(languageId);
	}

	public RumMapDTO[] getRumMapList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<RumMapDTO> rumMap = new RumMapDAS().findByEntity(entity_id, limit);
		return rumMap == null ? null : rumMap.toArray(new RumMapDTO[rumMap
				.size()]);
	}

	// ConvergentRateCard
	public Integer createRate(CRateWS rate) throws SessionInternalError {
		Integer rateId = 0;
		try {
			LOG.debug("in create rate method in wsssb...");
			CRateBL bl = new CRateBL();
			CRateDTO dto = new CRateDTO();
			LOG.debug("cratebl.create method is called...");
			rateId = bl.create(rate.getPrefix(), rate.getDestination(),
					rate.getFlatRate(), rate.getconncharge(),
					rate.getscaledRate(), rate.getratePlan(),
					rate.getCreatedDate(), rate.getValidFrom(),
					rate.getValidTo(), rate.getLastUpdatedDate(),
					rate.getRateType(), rate.getEntity(), rate.getCallType());
	...
	public void deleteConvergentRate(Integer id) throws SessionInternalError {
	...
	public CRateWS getCRateWS(Integer rateId) throws SessionInternalError {
		...
		CRateDAS das = new CRateDAS();
		CRateDTO rate = das.find(rateId);
		if (rate == null) {
			return null;
		}
		CRateBL bl = new CRateBL(rate);
		return bl.getCRateWS(languageId);
	}

	public CRateDTO[] getCRateList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<CRateDTO> rates = new CRateDAS().findByEntity(entity_id, limit);
		return rates == null ? null : rates.toArray(new CRateDTO[rates.size()]);
	}

	public Integer createDestinationMap(DestinationMapWS destinationMapWS)
			throws SessionInternalError {
		Integer destinationMapId = 0;
		try {
			LOG.debug("in create destination map method in wsssb..."
					+ destinationMapWS.getMapGroup());
			DestinationMapBL bl = new DestinationMapBL();

			LOG.debug(" destination mapbl.create method is called..."
					+ destinationMapWS.getCategory());
			destinationMapId = bl.create(destinationMapWS.getMapGroup(),
					destinationMapWS.getPrefix(),
					destinationMapWS.getTierCode(),
					destinationMapWS.getDescription(),
					destinationMapWS.getCategory(), destinationMapWS.getRank());
	...
	public void deleteDestinationMap(Integer id) throws SessionInternalError {
	...
	public DestinationMapWS getDestinationMapWS(Integer destinationMapId)
			throws SessionInternalError {

		if (destinationMapId == null || destinationMapId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		DestinationMapDAS das = new DestinationMapDAS();
		DestinationMapDTO destinationMapDTO = das.find(destinationMapId);
		if (destinationMapDTO == null) {
			return null;
		}
		DestinationMapBL bl = new DestinationMapBL(destinationMapDTO);
		return bl.getDestinationMapWS(languageId);
	}

	public Integer createHolidayMap(HolidayMapWS holidayMapWS)
			throws SessionInternalError {
		Integer holidayMapId = 0;
		try {
			LOG.debug("in create holiday map method in wsssb..."
					+ holidayMapWS.getMapGroup());
			HolidayMapBL bl = new HolidayMapBL();

			LOG.debug(" holiday mapbl.create method is called..."
					+ holidayMapWS.getYear());
			holidayMapId = bl.create(holidayMapWS.getMapGroup(),
					holidayMapWS.getDay(), holidayMapWS.getMonth(),
					holidayMapWS.getYear(), holidayMapWS.getDescription());
	...
	public void deleteHolidayMap(Integer id) throws SessionInternalError {
	...
	public HolidayMapWS getHolidayMapWS(Integer holidayMapId)
			throws SessionInternalError {

		if (holidayMapId == null || holidayMapId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		HolidayMapDAS das = new HolidayMapDAS();
		HolidayMapDTO holidayMapDTO = das.find(holidayMapId);
		if (holidayMapDTO == null) {
			return null;
		}
		HolidayMapBL bl = new HolidayMapBL(holidayMapDTO);
		return bl.getHolidayMapWS(languageId);
	}

	public Integer createUploadCDRFile(UploadCDRFileWS UploadCDRFileWS)
			throws SessionInternalError {
		Integer UploadCDRFileId = 0;
		try {
			// LOG.debug("in create holiday map method in wsssb..."+holidayMapWS.getMapGroup());
			UploadCDRFileBL bl = new UploadCDRFileBL();

			// LOG.debug(" holiday mapbl.create method is called..."+holidayMapWS.getYear());
			UploadCDRFileId = bl.create(UploadCDRFileWS.getName(),
					UploadCDRFileWS.getDate(), UploadCDRFileWS.getStatus(),
					UploadCDRFileWS.getType());

		} catch (Exception e) {
			e.printStackTrace();
		}
		return UploadCDRFileId;
	}

	public void deleteUploadCDRFile(Integer id) throws SessionInternalError {
	...
	public UploadCDRFileWS getUploadCDRFileWS(Integer UploadCDRFileId)
			throws SessionInternalError {

		if (UploadCDRFileId == null || UploadCDRFileId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		UploadCDRFileDAS das = new UploadCDRFileDAS();
		UploadCDRFileDTO UploadCDRFileDTO = das.find(UploadCDRFileId);
		if (UploadCDRFileDTO == null) {
			return null;
		}
		UploadCDRFileBL bl = new UploadCDRFileBL(UploadCDRFileDTO);
		return bl.getUploadCDRFileWS(languageId);
	}

	public Integer createWorldZoneMap(WorldZoneMapWS worldZoneMapWS)
			throws SessionInternalError {
		Integer worldZoneMapId = 0;
		try {
			LOG.debug("in create destination map method in wsssb..."
					+ worldZoneMapWS.getMapGroup());
			WorldZoneMapBL bl = new WorldZoneMapBL();

			LOG.debug(" destination mapbl.create method is called..."
					+ worldZoneMapWS.getTierCode());
			worldZoneMapId = bl
					.create(worldZoneMapWS.getMapGroup(),
							worldZoneMapWS.getTierCode(),
							worldZoneMapWS.getWorldZone());

		} catch (Exception e) {
			e.printStackTrace();
		}
		return worldZoneMapId;
	}

	public void deleteWorldZoneMap(Integer id) throws SessionInternalError {
	...
	public WorldZoneMapWS getWorldZoneMapWS(Integer worldZoneMapId)
			throws SessionInternalError {

		if (worldZoneMapId == null || worldZoneMapId == 0)
			return null;

		UserBL userbl = new UserBL(getCallerId());
		Integer languageId = userbl.getEntity().getLanguageIdField();

		WorldZoneMapDAS das = new WorldZoneMapDAS();
		WorldZoneMapDTO worldZoneMapDTO = das.find(worldZoneMapId);
		if (worldZoneMapDTO == null) {
			return null;
		}
		WorldZoneMapBL bl = new WorldZoneMapBL(worldZoneMapDTO);
		return bl.getWorldZoneMapWS(languageId);
	}

	public Integer uploadRates(List<RateWS> rates) throws SessionInternalError {
		Integer flag = 0;
		try {

			RateBL bl = new RateBL();
			RateDTO dto = new RateDTO();
			for (RateWS rate : rates) {
				Integer rateId = bl.create(rate.getPrefix(),
						rate.getDestination(), rate.getFlatRate(),
						rate.getconncharge(), rate.getscaledRate(),
						rate.getratePlan(), rate.getCreatedDate(),
						rate.getValidFrom(), rate.getValidTo(),
						rate.getLastUpdatedDate(), rate.getRateType(),
						rate.getEntity());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Integer uploadCrates(List<CRateWS> crates)
		...
		try {
			...
			CRateBL bl = new CRateBL();
			CRateDTO dto = new CRateDTO();
			for (CRateWS crate : crates) {
				Integer crateId = bl.create(crate.getPrefix(),
						crate.getDestination(), crate.getFlatRate(),
						crate.getconncharge(), crate.getscaledRate(),
						crate.getratePlan(), crate.getCreatedDate(),
						crate.getValidFrom(), crate.getValidTo(),
						crate.getLastUpdatedDate(), crate.getRateType(),
						crate.getEntity(), crate.getCallType());
			}
		} catch (Exception e) {
			e.printStackTrace();
			flag = 1;
	...
	// Exclusions
	public Integer createExclusion(ExclusionWS exclusion)
			throws SessionInternalError {
		Integer exclusionId = 0;
		try {
			LOG.debug("in create Exclusion method in wsssb...");
			ExclusionBL bl = new ExclusionBL();
			ExclusionDTO dto = new ExclusionDTO();
			LOG.debug("ExclusionBL.create method is called...");
			exclusionId = bl.create(exclusion.getPrefix(),
					exclusion.getDestination(), exclusion.getField1(),
					exclusion.getField2(), exclusion.getratePlan(),
					exclusion.getCreatedDate(), exclusion.getValidFrom(),
					exclusion.getValidTo(), exclusion.getLastUpdatedDate(),
					exclusion.getEntity());
	...
	public void deleteExclusion(Integer id) throws SessionInternalError {
	...
	public ExclusionWS getExclusionWS(Integer exclusionId)
			throws SessionInternalError {
		...
		ExclusionDAS das = new ExclusionDAS();
		ExclusionDTO exclusion = das.find(exclusionId);
		if (exclusion == null) {
			return null;
		}
		ExclusionBL bl = new ExclusionBL(exclusion);
		return bl.getExclusionWS(languageId);
	}

	public ExclusionDTO[] getExclusionList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<ExclusionDTO> exclusions = new ExclusionDAS().findByEntity(
				entity_id, limit);
		return exclusions == null ? null : exclusions
				.toArray(new ExclusionDTO[exclusions.size()]);
	}

	public void updateDeviceStatus(DeviceDTO dto, Integer newStatus) {

		DeviceDAS das = new DeviceDAS();
		DeviceDTO device = das.find(dto.getId());
		Integer oldStatus = device.getDeviceStatus().getId();
		if (oldStatus == newStatus) {
			/* No Change in status */
			return;
		}
		device.setDeviceStatus(new DeviceStatusDAS().find(newStatus));
	...
	public boolean removeFeature(String MSISDN, Integer featureId) {

		final int SuccessfulCompletion = 0;
		ServiceFeatureDTO feature = new ServiceFeatureDAS().find(featureId);
		List<ServiceFeatureDTO> features = new ArrayList<ServiceFeatureDTO>();
		...
		LOG.debug("Calling TDC API updateSubscriberRemoveService for removeFeature");

		/*
		 * UpdateSubscriberReplyInfo reply = new
		 * ServiceBL().updateSubscriberRemoveService(MSISDN, features);
		 * 
		 * StatusMessage status = reply.getStatusMessage();
		 * 
		 * if (status.getReturnCode() == SuccessfulCompletion) { return true;
		 * 
		 * 
		 * } else { return false;
		 * 
	...
	public boolean addFeature(String MSISDN, Integer featureId) {
		...
		LOG.debug("Calling TDC API updateSubscriberAddService for AddFeature");

		/*
		 * UpdateSubscriberReplyInfo reply = new
		 * ServiceBL().updateSubscriberAddService(MSISDN, features);
		 * 
		 * StatusMessage status = reply.getStatusMessage();
		 * 
		 * if (status.getReturnCode() == SuccessfulCompletion) { return true;
	...
	public void activateService(Integer serviceId) {
		LOG.debug("In WSSB activateService ");
		LOG.debug("In activateService serviceId" + serviceId);
		new ServiceBL().activateService(serviceId);
	}

	public void inactivateService(Integer serviceId) {
		LOG.debug("In WSSB inactivateService ");
		LOG.debug("In inactivateService serviceId" + serviceId);
		new ServiceBL().inactivateService(serviceId);
	}

	public ServiceWS getService(Integer serviceId) throws SessionInternalError {
		// get the info from the caller
		Integer languageId = getCallerLanguageId();

		// now get the service. Avoid the proxy since this is for the client
		ServiceDAS das = new ServiceDAS();
		ServiceDTO service = das.findNow(serviceId);
		if (service == null) { // not found
			return null;
		}
		ServiceBL bl = new ServiceBL(service);
		if (service.getDeleted() == 1) {
			LOG.debug("Returning deleted service " + serviceId);
		}
		return bl.getWS(languageId);
	}
	...
	public List<ServiceWS> getSubscriptionList(Integer userId) {
		...
		sBl.setUserId(userId);
		List<ServiceWS> subsList = sBl.getAllSubscriptions();

		return subsList;
	}

	public UserBalanceWS getUserBalance(Integer balanceId)
			throws SessionInternalError {
		// get the info from the caller
		Integer languageId = getCallerLanguageId();

		BalanceDAS das = new BalanceDAS();
		BalanceDTO balance = das.findNow(balanceId);
		if (balance == null) { // not found
			return null;
		}
		BalanceBL bl = new BalanceBL(balance);
		if (balance.getDeleted() == 1) {
			LOG.debug("Returning deleted balance " + balance.getId());
		}
		return bl.getWS(languageId);
	}

	public BalanceWS[] getUserBalances(Integer userId, Integer orderId,
			Integer orderLineId) throws SessionInternalError {
		// get the info from the caller
		String telephone = null;
		Integer languageId = getCallerLanguageId();

		BalanceBL bl = new BalanceBL();
		List<BalanceDTO> balances = bl.getValidUserBalances(userId, orderId,
				orderLineId);
		if (balances == null) { // not found
			return null;
		}
		List<BalanceWS> ws = new ArrayList<BalanceWS>(balances.size());

		UserDeviceDAS das = new UserDeviceDAS();
		try {
			telephone = das.findUserDeviceByOrderAndLine(orderId, orderLineId)
					.getTelephoneNumber();
		} catch (Exception e) {
			LOG.error("telephone number not found");
		}
		for (BalanceDTO balance : balances) {
			UserBalanceBL uBl = new UserBalanceBL(balance.getBaseUserByUserId()
					.getId(), balance.getCurrency().getId(), telephone);
			LOG.debug("User current balance for resource "
					+ balance.getCurrency().getId() + "is "
					+ uBl.getCurrentBalance());

			ws.add(new BalanceWS(balance.getId(),
					balance.getBaseUserByUserId(), balance.getCurrency(),
					balance.getOrderLineDTO(), balance.getOrderDTO(), balance
							.getBalance(), uBl.getCurrentBalance(), balance
							.getActiveSince(), balance.getActiveUntil(),
					balance.getCreateDate(), balance.getDeleted()));

		}

		return ws.toArray(new BalanceWS[ws.size()]);

	}

	public UserDeviceWS getUserDeviceWSById(Integer userDeviceId)
			throws SessionInternalError {
		...
		UserDeviceDAS das = new UserDeviceDAS();
		UserDeviceDTO userDevice = new UserDeviceDAS().find(userDeviceId);
		if (userDevice == null) { // not found
			return null;
		}

		UserDeviceWS userDeviceWS = new UserDeviceWS(userDevice.getId(),
				userDevice.getDevice(), userDevice.getBaseUser(),
				userDevice.getUserDeviceStatus(), userDevice.getCreatedDate(),
				userDevice.getDeleted(), userDevice.getOptlock(),
				userDevice.getTelephoneNumber(), userDevice.getIcc());

		return userDeviceWS;
	}

	public UserDeviceWS getUserDeviceByOrderAndLine(Integer orderId,
			Integer orderLineId) throws SessionInternalError {

		UserDeviceBL bl = new UserDeviceBL();
		return bl.getUserDeviceByOrderAndLine(orderId, orderLineId);
	}

	public ServiceFeatureDTO[] getServiceFeatures(Integer serviceId)
			throws SessionInternalError {

		ServiceFeatureBL serviceFeatureBl = new ServiceFeatureBL();

		List<ServiceFeatureDTO> serviceFeatureDtoList = serviceFeatureBl
				.getServiceFeaturesbyService(serviceId);

		ServiceFeatureDTO[] serviceFeature = serviceFeatureDtoList
				.toArray(new ServiceFeatureDTO[serviceFeatureDtoList.size()]);
	...
	public Integer addServiceAlias(Integer serviceId, String aliasName)
			throws SessionInternalError {
		Integer NewServicealiseId = 0;
		try {
			ServiceBL sbl = new ServiceBL();
			NewServicealiseId = sbl.addAlias(serviceId, aliasName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return NewServicealiseId;
	}

	public Integer addServiceAliasByOrder(Integer orderId, String aliasName)
			throws SessionInternalError {
		Integer ServicealiseId = 0;
		try {
			LOG.debug("OrderId in AddServicealiasByOder" + orderId);
			ServiceDTO sDto = new ServiceDAS().findByOrder(orderId);
			Integer sid = sDto.getId();
			LOG.debug("ServiceId in AddServicealiasByOder" + sid);
			ServicealiseId = addServiceAlias(sid, aliasName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ServicealiseId;
	}

	public void deleteAliasName(Integer id) throws SessionInternalError {
		// now get the order
	...
	public ServiceAliasDTO[] getServiceAlias(Integer serviceId)
			throws SessionInternalError {

		ServiceBL serviceAliasBl = new ServiceBL();

		List<ServiceAliasDTO> serviceAliasDtoList = serviceAliasBl
				.getServiceAliasbyService(serviceId);

		ServiceAliasDTO[] serviceAlias = serviceAliasDtoList
				.toArray(new ServiceAliasDTO[serviceAliasDtoList.size()]);

		return serviceAlias;
	}

	public ItemDTO getPlanBySubscriptionId(Integer subscriptionId) {
		return new ServiceBL().getPlanById(subscriptionId);
	}

	public void changeSubscriptionPlan(Integer subscriptionId,
			Integer fromItemId, Integer toItemId, Date changeDate) {

		new ServiceBL().changeSubscriptionPlan(subscriptionId, fromItemId,
				toItemId, changeDate);
	...
	public Integer createTicket(SupportTicketWS ticketWS) {
		...
		LOG.debug("base id is " + executorId);
		if (executorId != null) {
			UserDTO assignedUserDTO = user.find(executorId);
			// ticketWS.setAssignedUser(assignedUserDTO);
			ticketWS.setAssignedUserID(executorId);
		}
		return new TicketBL().createTicket(ticketWS);
	}

	public Integer updateTicket(SupportTicketWS ticketWS) {
		return new TicketBL().updateTicket(ticketWS);
	}

	public List<SupportTicketWS> getTicketDetailsByUser(String userName) {
		LOG.debug("I am in WebServices method [Method : getTicketDetailsByUser()]");
		List<SupportTicketWS> testWS = new TicketBL()
				.getTicketDetails(userName);
		for (SupportTicketWS test : testWS) {
			LOG.debug("Ticket Details in WS : " + test);
		}
		return testWS;
	}

	public List<SupportTicketWS> getTicketBodyUsingTicket(Integer ticketID) {
		LOG.debug("I am in WebServices method [Method : getTicketBodyUsingTicket()]");
		List<SupportTicketWS> testWS = new TicketBL()
				.getTicketBodyUsingTicket(ticketID);
		for (SupportTicketWS test : testWS) {
			LOG.debug("Ticket Details in WS : " + test);
		}
		return testWS;
	...
	public Integer createDevices(DeviceWS device) throws SessionInternalError {

		DeviceBL bl = new DeviceBL();
		DeviceDTO dto = new DeviceDTO();
		Integer deviceId = bl.create(getCallerCompanyId(), device.getIcc(),
				device.getImsi(), device.getPuk1(), device.getPuk2(),
				device.getPin1(), device.getPin2(), device.getCreatedDate(),
				device.getLastUpdatedDate());
		return deviceId;

	}

	public void deleteDevice(Integer id) throws SessionInternalError {
		DeviceBL bl = new DeviceBL();
	...
	// getdeviceWS method with PIN1, PIN2
	public DeviceWS getDeviceWS(Integer deviceId) throws SessionInternalError {
		...
		DeviceDAS das = new DeviceDAS();
		DeviceDTO device = das.find(deviceId);
		if (device == null) { // not found
			return null;
		}

		DeviceBL bl = new DeviceBL(device);
		return bl.getDeviceWS(languageId);
	}

	// getdeviceWS method with PIN1, PIN2
	public DeviceWS getDevicesWS(Integer deviceId) throws SessionInternalError {
		...
		DeviceBL bl = new DeviceBL(device);
		return bl.getDevicesWS(languageId);
	}

	public DeviceDTO getDeviceDTO(String ICC) {
		return new DeviceDAS().findByICC(ICC);
	}

	public DeviceDTO[] getDeviceList(Integer entity_id, Integer limit)
			throws NamingException, SQLException {

		List<DeviceDTO> devices = new DeviceDAS()
				.findByEntity(entity_id, limit);
		return devices == null ? null : devices.toArray(new DeviceDTO[devices
				.size()]);
	}

	public boolean changeICC(UserDeviceWS userDeviceWS, String newICC) {

		LOG.debug("In WSSB's changeICC method");
		boolean retValue = new UserDeviceBL().changeICC(userDeviceWS, newICC);
		return retValue;
	}

	public boolean cancelSubscription(String userId, Date reqDate) {
		// LOG.debug("");
		// boolean retValue= true;
		boolean retValue = new ServiceBL().cancelSubscription(userId, reqDate);
		return retValue;
	...
	public Integer createSubscription(OrderWS newOrder,
		...
		try {
			newOrderId = createOrder(newOrder, orderChange);
			LOG.debug("OrderId in createSubscription " + newOrderId);
			Integer newServicealiasId = addServiceAliasByOrder(newOrderId,
					aliasName);
			LOG.debug("newServicealiasId in createSubscription "
					+ newServicealiasId);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void createSubscriptionWithAliasList(OrderWS order,
		...
		Integer newOrderId = null;
		try {
			newOrderId = createOrder(order, orderChange);
			addServiceAliasListByOrder(newOrderId, aliasList);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public void addServiceAliasListByOrder(Integer orderId,
	...
	public Integer addDevice(Integer serviceId, Integer deviceId)
			throws SessionInternalError {
		Integer NewDeviceId = 0;
		LOG.debug("Adding new device :" + deviceId);
		try {
			ServiceBL sbl = new ServiceBL();
			NewDeviceId = sbl.addDevice(serviceId, deviceId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return NewDeviceId;
	...
	public Integer addUserDeviceByOrderAndLine(Integer orderId, Integer deviceId)
			throws SessionInternalError {
		Integer newDeviceId = 0;
		try {
			ServiceDTO sDto = new ServiceDAS().findByOrder(orderId);
			Integer sid = sDto.getId();
			newDeviceId = addDevice(sid, deviceId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return newDeviceId;
	...
	public Integer addServiceSite(Integer serviceId, String siteAddr)
			throws SessionInternalError {
		Integer NewServiceSiteId = 0;
		LOG.debug("Adding new Site :" + siteAddr);
		try {
			ServiceBL sbl = new ServiceBL();
			NewServiceSiteId = sbl.addSite(serviceId, siteAddr);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return NewServiceSiteId;
	}

	public Integer addServiceSiteByOrder(Integer orderId, String siteAddr)
			throws SessionInternalError {
		Integer ServiceSiteId = 0;
		try {
			ServiceDTO sDto = new ServiceDAS().findByOrder(orderId);
			Integer sid = sDto.getId();
			ServiceSiteId = addServiceSite(sid, siteAddr);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ServiceSiteDTO[] getServiceSites(Integer serviceId)
			throws SessionInternalError {

		ServiceBL serviceAliasBl = new ServiceBL();

		List<ServiceSiteDTO> serviceSiteDtoList = serviceAliasBl
				.getServiceSitebyService(serviceId);

		ServiceSiteDTO[] serviceSites = serviceSiteDtoList
				.toArray(new ServiceSiteDTO[serviceSiteDtoList.size()]);
	...
	// user device functions for subscription
	public Integer addServiceDeviceByOrder(Integer orderId, Integer deviceId)
			throws SessionInternalError {
		Integer ServiceDeviceId = 0;
		try {
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Integer addServiceDevice(Integer serviceId, Integer deviceId)
			throws SessionInternalError {
		Integer NewUserDeviceId = 0;
		LOG.debug("Adding new Device :" + deviceId);
		try {
			ServiceBL sbl = new ServiceBL();
			NewUserDeviceId = sbl.addDevice(serviceId, deviceId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return NewUserDeviceId;
	}

	public UserDeviceDTO[] getServiceDevices(Integer serviceId)
			throws SessionInternalError {

		ServiceBL serviceAliasBl = new ServiceBL();

		List<UserDeviceDTO> userDeviceDtoList = serviceAliasBl
				.getUserDeviceByService(serviceId);

		UserDeviceDTO[] userDevices = userDeviceDtoList
				.toArray(new UserDeviceDTO[userDeviceDtoList.size()]);

		return userDevices;
	}

	public void deleteUserevice(Integer id) throws SessionInternalError {
		ServiceBL bl = new ServiceBL();
		bl.deleteDevice(id);
	}
	...
	public Integer purchaseBundleUsinBundleId(Integer bundleId, Integer userId) {
		...
		LOG.debug("Given Bundle " + bundleId + " and User " + userId);
		PricePackageWS bundle = getBundleWS(bundleId);
		LOG.debug("Bundle WS Object : " + bundle);
		PurchasedBundleWS purchasedBundleWS = new PurchasedBundleWS();
		purchasedBundleWS.setBundleId(bundleId);
		LOG.debug("bundle.getActiveSince()" + bundle.getActiveSince());
		...
		LOG.debug("bundle  name " + bundle.getBundleName());
		purchasedBundleWS.setBundleName(bundle.getBundleName());
		LOG.debug("user id  : " + userId);
		purchasedBundleWS.setUserDto(new UserDAS().find(userId));
		Integer purchasedBundleId = purchaseBundle(purchasedBundleWS);
		return purchasedBundleId;
	}

	public Integer purchaseBundle(PurchasedBundleWS purchasedBundleWS) {

		PurchasedBundleBL bl = new PurchasedBundleBL();
		Integer purchaseBundleId = bl.createPurchaseBundle(purchasedBundleWS);
		Integer purchaseBundleProductId = bl.createPurchaseBundleProduct(
				purchasedBundleWS, purchaseBundleId);

		return purchaseBundleProductId;
	}

	public static Date getDateByOffSet(Integer offset, Integer value) {
	...
	public PurchasedBundleWS getPurchasedBundle(Integer id) {
		PurchasedBundleBL bl = new PurchasedBundleBL();
		PurchasedBundleWS ws = bl.getPurchasedBundleWS(id);
		return ws;
	}

	public void cancelBundle(Integer purchaseId, Date reqDate) {
		PurchasedBundleBL bl = new PurchasedBundleBL();
		bl.cancelBundle(purchaseId, reqDate);
	}

	public static PackageProductDTO getDto(PackageProductWS packageProductWS) {
		if (packageProductWS != null) {
			PackageProductDTO packageProduct = new PackageProductDTO(
					packageProductWS);
	...
	public static List<PackageProductDTO> getPackageProductDTO(
			PackageProductWS[] objects) {
		List<PackageProductDTO> packageProductDto = new ArrayList<PackageProductDTO>(
				objects.length);
		for (PackageProductWS packageProductWS : objects) {
			packageProductDto.add(getDto(packageProductWS));
		}
		return packageProductDto;
	}

	public Integer createBundle(PricePackageWS bundle)
			throws SessionInternalError {
		LOG.debug(" bundle wssb" + bundle);
		PricePackageDTO dto = new PricePackageDTO();
		...
		if (bundle.getMbgCheck() != null) {
			if (bundle.getMbgCheck().equals("on")) {
		...
		dto.setEntity(cdto);
		// LOG.debug(" WSSSB bundle metafield : " +bundle.getMetaFields());
		/*
		 * MetaFieldValueWS[] valuews = bundle.getMetaFields(); for(int i=0;
		 * i<valuews.length ; i++) {
		 * dto.setMetaField(entityId,valuews[i].getFieldName
		 * (),valuews[i].getValue()); LOG.debug("Bundle field :"+ i +
		 * "  field name is :" +valuews[i].getFieldName() + "value is :" +
		 * valuews[i].getValue()); }
		 */
		PricePackageBL bl = new PricePackageBL();
		Integer bundleId = bl.create(entityId, dto);
		LOG.debug(" id is" + bundleId);
		PackageProductBL productBL = new PackageProductBL();
		Integer productId = null;
		PackageProductWS[] pakcgeProductWs = bundle.getPackageProducts();
		LOG.debug("======pakcgeProductWs====" + pakcgeProductWs);
		for (PackageProductWS products : pakcgeProductWs) {
			PackageProductDTO packageProductDTO = getDto(products);
			productId = productBL.create(bundleId, packageProductDTO);
			LOG.debug(" product  id" + productId);
			PackageProductDAS packageProductDas = new PackageProductDAS();
			PackageProductDTO pdt = packageProductDas.find(productId);
			PackagePriceTypeBL packagePriceTypeBl = new PackagePriceTypeBL();
			List<Integer> typeIdList = null;
			Integer typeId = null;
			Integer priceId = null;
			PackagePriceBL priceBl = new PackagePriceBL();
			List<PackagePriceDTO> packagePriceDTOList = priceBl
					.getPackagePriceDTO(products, pdt, bundle);
			LOG.debug("====entering in the method getPackagePriceDTO========");
			LOG.debug("==========packagePriceDTOList========="
					+ packagePriceDTOList);
			PackagePriceDTO[] packagePriceDTO = packagePriceDTOList
					.toArray(new PackagePriceDTO[packagePriceDTOList.size()]);
			LOG.debug("====packagePriceDTO====" + packagePriceDTO);
			LOG.debug("=======METHOD2======");
			for (PackagePriceDTO priceDto : packagePriceDTO) {
				priceId = priceBl.create(priceDto);
	...
	public PricePackageWS getBundleWS(Integer bundleId) {
		PricePackageDTO dto = getBundle(bundleId);
		PricePackageBL bl = new PricePackageBL();
		PricePackageWS ws = bl.getBundleWS(dto);

		return ws;

	}

	public PricePackageDTO getBundle(Integer bundleId)
			throws SessionInternalError {
		// get the info from the caller
		Integer languageId = getCallerLanguageId();
		...
		LOG.debug(" PricePackageDTO  " + dto);

		return dto;
	}

	public void deleteBundle(Integer id) throws SessionInternalError {
		// now get the order
		PricePackageBL bl = new PricePackageBL();
		// bl.setForUpdate(id);
		bl.deleteBundle(id);
	...
	public void updateBundle(PricePackageWS bundle) throws SessionInternalError {
		PricePackageBL oldBundle = new PricePackageBL();
		oldBundle.setForUpdate(bundle.getId());
		PricePackageBL bl = new PricePackageBL();
		PricePackageDTO dto = bl.getDTO(bundle);
		Integer entityId = getCallerCompanyId();
		CompanyDTO cdto = new CompanyDAS().find(entityId);
		dto.setEntity(cdto);

		LOG.debug(" WSSSB update bundle metafield : " + bundle.getMetaFields());
		MetaFieldValueWS[] valuews = bundle.getMetaFields();
		/*
		 * for(int i=0; i<valuews.length ; i++) {
		 * dto.setMetaField(dto.getEntity(
		 * ).getId(),valuews[i].getFieldName(),valuews[i].getValue());
		 * LOG.debug("Bundle field :"+ i + "  field name is :"
		 * +valuews[i].getFieldName() + "value after update is :" +
		 * valuews[i].getValue()); }
		 */

		oldBundle.update(entityId, dto);
		PackageProductBL productBL = new PackageProductBL();
		Integer ProdId = null;
		for (PackageProductWS products : bundle.getPackageProducts()) {
			PackageProductDTO packageProductDTO = getDto(products);
			ProdId = productBL.update(packageProductDTO, bundle.getId());
			PackageProductDTO pdt = new PackageProductDAS().find(ProdId);
			PackagePriceBL priceBl = new PackagePriceBL();
			List<PackagePriceDTO> packagePriceDTOList = priceBl
					.getPackagePriceDTO(products, pdt, bundle);
			for (PackagePriceDTO priceDto : packagePriceDTOList) {
				priceBl.update(priceDto, ProdId);
			}
	...
	public boolean bulkEmail(String recepient, Integer message) {
		...
		try {
			...
			CompanyDTO entity = new CompanyDAS().find(getCallerCompanyId());

			Integer entityId = entity.getId();
			LOG.debug("Entity Id in bulksms :" + entityId);
			Integer languageId = entity.getLanguageId();
			LOG.debug("languageId in bulksms :" + languageId);

			NotificationMessageLineDTO notificationMessageLineDTO = new NotificationMessageLineDAS()
					.find(message);
			String notificationMessage = notificationMessageLineDTO
					.getContent();

			Integer notificationTypeId = notificationMessageLineDTO
					.getNotificationMessageSection().getNotificationMessage()
					.getNotificationMessageType().getId();

			MessageDTO dto = notificationSession.getDTO(notificationTypeId,
					languageId, entityId);

			notificationEmail = notificationSession.notificationEmail(
					recepient, dto, entity, notificationMessage);

		} catch (Exception e) {
			LOG.warn("Exception in web service: notifying by email " + e);
			notificationEmail = false;
		}
		return notificationEmail;
	}

	public String bulkSMS(String recepient, String message) {

		String bulkSms;
		int eventLogId;

		String concat = recepient + ":" + message;

		try {

			CompanyDTO company = new CompanyDAS().find(getCallerCompanyId());

			OpenBRMNotificationTask task = new OpenBRMNotificationTask();
			bulkSms = task.sendNotificationSMS(recepient, message);

			DataInsertionDAS data = new DataInsertionDAS();
			eventLogId = data.InsertData(concat);
			LOG.debug("message sent1" + eventLogId);
			...
			LOG.warn("Exception in web service SMS " + e);
	...
	public Integer createVoucher(VoucherWS voucherWS)
		...
		Integer voucherId = 0;
		try {
			LOG.debug("in create voucher method in wsssb...");
			VoucherBL bl = new VoucherBL();
			VoucherDTO dto = new VoucherDTO();
			LOG.debug("voucherBL.create method is called...");
			voucherId = bl.create(voucherWS.getCreatedDateTime(),
					voucherWS.getVoucherStatus(), voucherWS.getEntityId(),
					voucherWS.getSerialNo(), voucherWS.getPinCode(),
					voucherWS.getBatchId(), voucherWS.getProductId());
			LOG.debug("create voucher method called");
			LOG.debug("create voucher method called:" + voucherId);
	...
	public VoucherDTO[] getVoucherList(Integer entityId, Integer serialNo)
			throws NamingException, SQLException {

		List<VoucherDTO> vouchers = new VoucherDAS().findBySerialNo(entityId,
				serialNo);
		return vouchers == null ? null : vouchers
				.toArray(new VoucherDTO[vouchers.size()]);
	}
	...
	public VoucherWS getVoucherWS(Integer voucherId)
		...
		VoucherDAS das = new VoucherDAS();
		VoucherDTO voucher = das.find(voucherId);
		if (voucher == null) { // not found
			return null;
		}

		VoucherBL bl = new VoucherBL(voucher);
		return bl.getVoucherWS(languageId);
	}
	...
	public Integer RedeemVoucher(String pinCode, Integer userId) {
		...
		try {
			VoucherDTO vDto = new VoucherDAS().findCustomerByPin(pinCode);
			String batchId = vDto.getBatchId();
			Integer voucher = vDto.getVoucherStatus().getId();
			Integer serialNo = vDto.getSerialNo();

			// UserBL user = new UserBL(userId);
			UserDTO user = new UserDAS().find(userId);
			LOG.debug("userId in RedeemVoucher is : " + userId);

			if (userId != user.getId()) {
				LOG.debug("No Customer exists with given userId");
			}

			else if (!pinCode.equals(vDto.getPinCode())) {
				LOG.debug("PinCode Doesnt exist");
			}

			else if (!voucher.equals(Constants.VOUCHER_STATUS_NEW)) {
				LOG.debug("voucher already in use");
			}

			else {

				Integer entityId = getCallerCompanyId();
				VoucherBL vbl = new VoucherBL();
				voucherId = vbl.RedeemVoucher(vDto, userId);
				LOG.debug("Redeem Voucher has been called");
			}
	...
	public void updateVoucherStatus(VoucherStatusDTO status,
		...
		new VoucherBL().updateVoucherStatus(status, voucherDTO);
