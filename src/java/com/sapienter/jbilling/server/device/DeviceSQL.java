...
public interface DeviceSQL {
    ...
    static final String getByUserAndPeriod =
        ...
        "   and period_id = ? " +
        "   and deleted = 0";
    static final String getByUser=
            "select id " +
            "  from purchase_service " +
            " where user_id = ? " +
            "   and deleted = 0";
    static final String getByUserAndDevice =
            "select id " +
            "  from purchase_service " +
            " where user_id = ? " +
            "   and device_id = ? " +
            "   and deleted = 0";
    /* static final String getByDevice=
            "select id " +
            "  from purchase_service " +
            " where device_id = ? " +
            "   and deleted = 0";*/
    /* static final String getByEntity="select id " +
            "  from purchase_service " +
            " where entity_id = ? " +
            "   and deleted = 0";*/
