...
public class PaymentInformationBL extends ResultList implements CreditCardSQL{
	 ...
	 private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(PaymentInformationBL.class));
    ...
    public boolean useGatewayKey(PaymentInformationDTO instrument) {        
        return (Constants.PAYMENT_METHOD_GATEWAY_KEY.equals(instrument.getPaymentMethod().getId()) || getStringMetaFieldByType(instrument, MetaFieldType.GATEWAY_KEY) != null);
    }
    ...
    public Date getDateMetaFieldByType(PaymentInformationDTO instrument, MetaFieldType type) {
    	...
    	try {
    		if(value != null) {
    			return DateTimeFormat.forPattern(Constants.CC_DATE_FORMAT).parseDateTime((String) value.getValue()).toDate();
    		}
    	}catch(Exception e) {
    ...
    private Date getDateMetaFieldByType(PaymentInformationWS cc, MetaFieldType date) {
        ...
        try {
            if(value != null) {
                return DateTimeFormat.forPattern(Constants.CC_DATE_FORMAT).parseDateTime(value).toDate();
            }
        }catch(Exception e) {
            LOG.error("Error occurred while parsing credit card date object: %s",e);
        }
