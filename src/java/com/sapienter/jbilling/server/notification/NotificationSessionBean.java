...
public class NotificationSessionBean implements INotificationSessionBean {
     ...
     * @return
    */
	public Boolean notificationEmail(String recepient,MessageDTO message,CompanyDTO company,String content) throws SessionInternalError{
			Boolean retValue;
				LOG.debug("we are in NotificationSessionBean method");
		  try {
				retValue = notifies(recepient, message,company,content);
				LOG.debug("we are in NotificationSessionBean method1" + retValue);
			} catch (Exception e) {
    ...
    */
	...
	public Boolean notifies(String recepient, MessageDTO message,CompanyDTO company,String content) 
            throws SessionInternalError {
        
			Boolean retValue = new Boolean(true);
				LOG.debug("we are in NotificationSessionBean method notifies" + retValue);
        try {
		...
		// verify that the message is good
			...
			Integer languageId = company.getLanguageId();
					LOG.debug("languageId in bulksms :" + languageId);
				
            PluggableTaskManager taskManager = new PluggableTaskManager(entityId,Constants.PLUGGABLE_TASK_NOTIFICATION);
				LOG.debug("we are in NotificationSessionBean method notifies1" );         
			
				
			NotificationTask task =(NotificationTask) taskManager.getNextClass();
		   
		    NotificationMessageArchDAS messageHome = new NotificationMessageArchDAS();
			 
			 while (task != null) {
					NotificationMessageArchDTO messageRecord = messageHome.create(message.getTypeId(), sections);
              
             try {
					task.delivery(recepient, message,company,content);
						LOG.debug("we are in NotificationSessionBean method notifies3" );
                } catch (TaskException e) {
                    messageRecord.setResultMessage(Util.truncateString(e.getMessage(), 200));
                    LOG.error(e);
                    retValue = new Boolean(false);
                }
