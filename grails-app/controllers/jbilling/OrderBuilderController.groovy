    ...
    def editFlow = {//do not change this to method. Webflow will not be able to find it.
        ...
        initialize {
            action {
				...
				//deviceTab statements
				def device = new ArrayList<DeviceWS>()
				log.debug device
				
				def devicetypes = DeviceTypeDTO.list()
			   	Integer entityId = webServicesSession.getCallerCompanyId();
				def devicelist = new DeviceDAS().findAllByEntity(entityId)
				...
				flow.devicetypes = devicetypes
				flow.devicelist = devicelist
				flow.device = device
		...
		/**
         * Renders the Device tab panel.
         */
        showDevices {
            action {
			def device = conversation.device
			bindData(device, params)
						
			if(params.deviceType != null) {
			log.debug(" In if condition ")
			Integer dtype = Integer.valueOf(params.deviceType)
			log.debug(" params device dtype in devices is ${dtype}")
	        DeviceTypeDTO devicetypedto =  new DeviceTypeDAS().find(dtype)
		    log.debug("devcetypedto is ${devicetypedto}")
			
			def newdevice = new DeviceWS()
			def newDeviceLines = conversation.device as List 
			newdevice.setSerialNum(params.serialNum)
			newdevice.setDeviceType(devicetypedto)
			newdevice.setVendorCode(params.deviceType)
			newDeviceLines.add(newdevice)
			log.debug (" newDeviceLines is ${newDeviceLines}")
			conversation.device = newDeviceLines.toArray()
             }	
			 
			params.template = 'devices'
            }
            on("success").to("build")
        }
