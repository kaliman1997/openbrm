...
class CustomerController {
	...
	def getList2(filters, statuses, params) {
		params.max = params?.max?.toInteger() ?: pagination.max
		params.offset = params?.offset?.toInteger() ?: pagination.offset
		params.sort = params?.sort ?: pagination.sort
		params.order = params?.order ?: pagination.order
		List<String> contactFilters = ['contact.firstName', 'contact.lastName', 'contact.email', 'contact.phoneNumber', 'contact.postalCode', 'contact.organizationName']
		...
		return UserDTO.createCriteria().list(
				...
				offset: params.offset
					...
					add(Restrictions.eq("metaField.fieldUsage", MetaFieldType.FIRST_NAME))

					def subCriteria = DetachedCriteria.forClass(StringMetaFieldValue.class, "stringMFValue")
							.setProjection(Projections.property('id'))
					subCriteria.add(Restrictions.ilike('stringMFValue.value', "Elida", MatchMode.ANYWHERE))
				...
				offset: params.offset
					...
					and {
						if(!Collections.disjoint(contactFilters,filters.findAll{it.value}*.field)) {
							...
							setFetchMode("mfv", FetchMode.JOIN)
							createAlias("mfv.field", "metaField")
							log.debug "CBOSS:: despues createAliasMFV"
							...
							or{
								filters.each { filter ->
									if (filter.value && contactFilters.contains(filter.field)) {
										...
										def subCriteria = DetachedCriteria.forClass(StringMetaFieldValue.class, "stringMFValue")
												.setProjection(Projections.property('id'))
										log.debug "CBOSS:: despues de subCriteria"
										...
										log.debug "CBOSS:: despues de if"
										Criterion aitMfv = Property.forName("mfv.id").in(subCriteria)
										
										LogicalExpression andExp = Restrictions.and(aitMfv, 
											Restrictions.eq("metaField.fieldUsage", filterService.getMetaFieldTypeForFilter(filter.field)))
										
										add(andExp)

										log.debug "CBOSS:: despues de addToCriteria"
									}
								}
							}
						}
	...
	def retrieveUserData(def user) {
		...
		def statusOnNetwork = 'NA'

		for(MetaFieldValue mfv :values){
			if(mfv.getField().getName().equals("MSISDN")){
				String MSISDN = (String) mfv.getValue();

				if(!MSISDN.equalsIgnoreCase("----")){
					ProfileResponse pr = new Profile().profile(MSISDN);

					if (pr.status.equalsIgnoreCase('success')){
						statusOnNetwork = pr.subStatus
					} else {
						statusOnNetwork = pr.description
					}
				}
			}
		...
		for(MetaFieldValue mfv :values){
			...
			if(mfv.getField().getName().equals("Nombre")){
				firstName = (String) mfv.getValue();
			} else if(mfv.getField().getName().equals("Apellidos")){
				lastName  = (String) mfv.getValue();
			}
		}
		...
		return [ revenue:revenue,
			...
			fullName : fullName,
			statusOnNetwork: statusOnNetwork
		]
	}
