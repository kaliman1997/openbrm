...
<%@ page
	import="org.apache.commons.lang.StringEscapeUtils; grails.plugin.springsecurity.SpringSecurityUtils; com.sapienter.jbilling.server.invoice.db.InvoiceStatusDAS; com.sapienter.jbilling.server.customer.CustomerBL; com.sapienter.jbilling.common.Constants; com.sapienter.jbilling.server.user.UserBL;"%>
		...
		<table class="dataTable" cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					...
					<td class="value"><sec:access url="/customerInspector/inspect">
							<g:link controller="customerInspector" action="inspect"
								id="${selected.id}"
								title="${message(code: 'customer.inspect.link')}">
								${selected.id}
								<img src="${resource(dir: 'images', file: 'magnifier.png')}"
									alt="inspect customer" />
							</g:link>
						</sec:access> <sec:noAccess url="/customerInspector/inspect">
							${selected.id}
						</sec:noAccess></td>
				</tr>
				<tr>
					...
					<td class="value"><g:if
							test="${!SpringSecurityUtils.isSwitched() && selected.id != session['user_id']}">
							<form id="switch-user-form"
								action="${request.contextPath}/j_spring_security_switch_user"
								method="POST">
								<g:hiddenField name="j_username"
									value="${selected.userName};${session['company_id']}" />
							</form>
							<a onclick="$('#switch-user-form').submit()"
								title="${message(code: 'switch.user.link')}"> ${selected.userName}
								<img src="${resource(dir: 'images', file: 'user_go.png')}"
								alt="switch user" />
							</a>
						</g:if> <g:else>
							${selected.userName}
				...
				<g:if test="${customer.partner}">
					<tr>
						...
						<td class="value"><g:remoteLink controller="partner"
								action="show" id="${customer.partner.id}"
				...
				<tr>
					<td><g:message code="cboss.altan.msisdn.status" /></td>
					<td class="value">${statusOnNetwork}</td>
				</tr>

				<tr>
					<td><g:message code="user.locked" /></td>
					<td class="value"><g:formatBoolean
							boolean="${selected.isAccountLocked()}" true="Yes" false="No" /></td>
				</tr>
				<tr>
					<td><g:message code="customer.detail.user.created.date" /></td>
					<td class="value"><g:formatDate
							date="${selected.createDatetime}" formatName="date.pretty.format" /></td>
				</tr>
				<tr>
					<td><g:message code="customer.detail.userCodes" /></td>
					<g:if test="${userCodes}">
						<td class="value"><g:set var="ucLength"
								value="${userCodes.size()-1}" /> <g:each in="${userCodes}"
								var="uc" status="ucIdx">
								<g:remoteLink controller="user" action="show"
									params="[userCode: uc]" before="register(this);"
									onSuccess="render(data, next);" method="GET">
									${uc}
								</g:remoteLink>
				...
				<g:if test="${metaFields}">
					...
					<g:render template="/metaFields/metaFields"
				...
				<g:if test="${customer?.parent}">
					...
					<tr>
						...
						<td class="value"><g:remoteLink action="show"
								id="${customer.parent.baseUser.id}" before="register(this);"
					...
					<tr>
						...
						<td class="value"><g:if test="${customer.invoiceChild > 0}">
								<g:message code="customer.invoice.if.child.true" />
							</g:if> <g:else>
								<g:set var="parent"
									value="${new CustomerBL(customer.id).getInvoicableParent()}" />
								<g:remoteLink action="show" id="${parent.baseUser.id}"
									before="register(this);" onSuccess="render(data, next);">
									<g:message code="customer.invoice.if.child.false"
										args="[ parent.baseUser.id ]" />
								</g:remoteLink>
							</g:else></td>
					</tr>
					<tr>
						...
						<td class="value"><g:formatBoolean
								boolean="${customer.useParentPricing}" /></td>
					</tr>
				</g:if>
				...
				<g:if test="${customer?.children}">
					...
					<!-- direct sub-accounts -->
					<g:each var="account"
						in="${customer.children.findAll{ it.baseUser.deleted == 0 }}">
						<tr>
							<td><g:message code="customer.subaccount.title"
									args="[ account.baseUser.id ]" /></td>
							<td class="value"><g:remoteLink action="show"
									id="${account.baseUser.id}" before="register(this);"
									onSuccess="render(data, next);">
		...
		<table class="dataTable" cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					...
					<td class="value"><sec:access url="/order/show">
							<g:remoteLink controller="order" action="show"
								id="${latestOrder?.id}" before="register(this);"
								onSuccess="render(data, next);">
								<g:formatDate date="${latestOrder?.createDate}"
									formatName="date.pretty.format" />
							</g:remoteLink>
						</sec:access> <sec:noAccess url="/order/show">
							<g:formatDate date="${latestOrder?.createDate}"
								formatName="date.pretty.format" />
						</sec:noAccess></td>
					<td class="value"><sec:access url="/order/list">
				...
				<tr>
					...
					<td class="value"><sec:access url="/invoice/show">
							<g:remoteLink controller="invoice" action="show"
								id="${latestInvoice?.id}" before="register(this);"
								onSuccess="render(data, next);">
								<g:formatDate date="${latestInvoice?.createDateTime}"
									formatName="date.pretty.format" />
							</g:remoteLink>
						</sec:access> <sec:noAccess url="/invoice/show">
							<g:formatDate date="${latestInvoice?.createDateTime}"
								formatName="date.pretty.format" />
						</sec:noAccess></td>
					<td class="value"><sec:access url="/invoice/list">
				...
				<tr>
					...
					<td class="value"><sec:access url="/payment/show">
							<g:remoteLink controller="payment" action="show"
								id="${latestPayment?.id}" before="register(this);"
								onSuccess="render(data, next);">
								<g:formatDate
									date="${latestPayment?.paymentDate ?: latestPayment?.createDatetime}"
									formatName="date.pretty.format" />
							</g:remoteLink>
						</sec:access> <sec:noAccess url="/payment/show">
							<g:formatDate
								date="${latestPayment?.paymentDate ?: latestPayment?.createDatetime}"
								formatName="date.pretty.format" />
						</sec:noAccess></td>
					<td class="value"><sec:access url="/payment/list">
				...
				<tr>
					...
					<td class="value"><g:set var="nextInvoiceDate"
							value="${customer.nextInvoiceDate}" /> <g:if
							test="${nextInvoiceDate}">
							<span><g:formatDate date="${nextInvoiceDate}"
									formatName="date.pretty.format" /></span>
						</g:if> <g:else>
				...
				<tr>
					...
					<td class="value"><g:set var="subscriptionPeriod"
							value="${customer?.mainSubscription?.subscriptionPeriod}" /> <g:set
							var="nextInvoiceDayOfPeriod"
							value="${customer?.mainSubscription?.nextInvoiceDayOfPeriod}" />
						<span>
				...
				<tr>
					...
					<td class="value"><g:formatDate
							date="${latestInvoice?.dueDate}" formatName="date.pretty.format" /></td>
				</tr>
				<tr>
					<td><g:message code="customer.detail.payment.invoiced.amount" /></td>
					<td class="value"><g:formatNumber
							number="${latestInvoice?.totalAsDecimal}" type="currency"
							currencySymbol="${selected.currency.symbol}" /></td>
				</tr>
				<tr>
					<td><g:message code="invoice.label.status" /></td>
					<td class="value"><g:if test="${latestInvoice}">
							<g:set var="invoiceStatus"
								value="${new InvoiceStatusDAS().find(latestInvoice?.statusId)}" />
							<g:if
								test="${latestInvoice?.statusId == Constants.INVOICE_STATUS_UNPAID && isCurrentCompanyOwning}">
								<g:link controller="payment" action="edit"
									params="[userId: selected.id, invoiceId: latestInvoice.id]"
									title="${message(code: 'invoice.pay.link')}">${invoiceStatus.getDescription(session['language_id'])}
                                    </g:link>
                                </g:if>
                                <g:else>
