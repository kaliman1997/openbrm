   ...
   <div class="table-scroll">
    <table id="pricemodel" cellspacing="0" cellpadding="0">
		...
		<g:remoteSort action="list" sort="id" update="column1">
			<g:message code="price.map.detail.id"/>
                </g:remoteSort>
		</th>
            <th class="large">
              <g:message code="price.map.detail.map.group"/>
	    </th>
           <th class="large">
		<g:message code="price.map.detail.price.group"/>
	   </th>
      
	   <th class="large">
	   <g:remoteSort action="list" sort="priceMapPlan" update="column1">
		<g:message code="price.map.detail.price.map.plan"/>
		</g:remoteSort>
	  </th>
	  
	  <th class="large">
		<g:message code="price.map.detail.rate.price"/>
	  </th>
	  <th class="large">
		<g:message code="price.map.detail.set.up.price"/>
	  </th>
	  </tr>
        </thead>
           ...
           <g:each var="iprice" in="${pricemap}">
                ...
                <tr id="type-1" class="active">
						...
						<td>
			                ...
			                <g:formatNumber number="${iprice.ratePrice}" type="currency" currencySymbol="${currency.symbol}" />
						...
						<td>
                            <g:remoteLink breadcrumb="id" class="cell" action="show" id="${iprice.id}" params="['template': 'show']" before="register(this);" onSuccess="render(data, next);">
							<g:formatNumber number="${iprice.setUpPrice}" type="currency" currencySymbol="${currency.symbol}" />
...
<div class="pager-box">
    ...
    <div class="row">
        <util:remotePaginate controller="priceMap" action="list" params="${sortableParams(params: [partial: true])}" total="${pricemap?.totalCount ?: 0}" update="column1"/>
    </div>
</div>
