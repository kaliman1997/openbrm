<%@ page import="org.apache.commons.lang.StringUtils; com.sapienter.jbilling.server.metafields.DataType;org.apache.commons.lang.WordUtils; com.sapienter.jbilling.server.user.db.CompanyDTO;	com.sapienter.jbilling.server.util.db.EnumerationDTO; com.sapienter.jbilling.common.Constants"%>
...
<g:each var="field" in="${availableFields?.sort{ it.displayOrder }}">
	...
	<g:if test="${!field.disabled}">
		<g:set var="fieldName"
			value="${StringUtils.abbreviate(message(code: field.name), 50)}" />
		<g:set var="fieldValue"
			value="${fieldValues?.find{ (it.fieldName == field.name) &&
		...
		<g:elseif
			test="${g.ifValuePresent(field:field,fieldsArray: fieldsArray)}">
			<g:set var="fieldValue"
				value="${g.setFieldValue(field:field, fieldsArray:fieldsArray)}" />
		</g:elseif>
		<!-- p>campo: '${fieldName}'</p -->
		<g:if test="${fieldName == Constants.METAFIELD_NAME_CC_NUMBER || 
			fieldName == 'Número de Tarjeta'}">
			<g:if
				test="${!user?.id || user?.id < 0 || !fieldValues?.getAt(0)?.id || fieldValues?.getAt(0)?.id == null}">
				<g:set var="fieldValue" value="${fieldValue}" />
			</g:if>
			<g:elseif test="${user?.id > 0}">
				<g:set var="fieldValue"
					value="${fieldValue?.replaceAll('^\\d{12}','************')}" />
			</g:elseif>
		</g:if>


		<g:if test="${fieldName != 'cc.token'}">

			<g:render id="dataTypeTemplate"
				template="/metaFields/dataType/${templateName}"
				model="[
